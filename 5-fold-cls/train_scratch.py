...
from MangoDatasetAugument import MangoDataset
...
def getData(batch_size, fold):
    ...
    train_data = MangoDataset(ALL_DIR, train_img_names,bbox=train_bbox, labels=train_one_hot, transform = train_transform, train=True)
    eval_data = MangoDataset(ALL_DIR, eval_img_names, bbox=eval_bbox, labels=eval_one_hot, transform = eval_transform, train=False)
...
def main(args):
    ...
    #model = ptcv_get_model("resnet10", pretrained=False)
