...
from TrainerDTN import DTNTrainer
...
def main(args):
    ...
    train_data, eval_data, pos_weight = getData(args.train_ratio, args.batch_size, args.seed)
    #sampler = BalancedSampler(train_data)
    #train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, sampler=sampler, num_workers=4)
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=4)
    gen_loader1 = torch.utils.data.DataLoader(train_data, batch_size=4, shuffle=True, num_workers=4)
    gen_loader2 = torch.utils.data.DataLoader(train_data, batch_size=4, shuffle=True, num_workers=4)

    eval_loader = torch.utils.data.DataLoader(eval_data, batch_size=args.batch_size, shuffle=False, num_workers=4)
    ...
    model.to(device)
    trainer = DTNTrainer(train_loader, gen_loader1, gen_loader2, eval_loader, model, device, ckpt_path)
